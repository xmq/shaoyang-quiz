...
面向事业单位专业知识备考，按“讲义理解 → 三色笔记按考频复习 → 专项刷题检验 → 到期复习巩固”的闭环组织内容。重点覆盖基础概念、特点与要素、作用与流程、判断与简答得分点，以及少量一步计算。

## 当前内容

- 题库总量：**2531 道**，其中公开练习 **2381 道**，已按同科规范化题干去重。
- 题型包括单选、多选、判断、填空和简答自评，公开题含 74 道简答；全部题目带答案与文字解析，计算题以子网划分、公式直接代入和单位判断为主。
- 公开题库聚焦基础与常见题型，已隐藏偏复杂综合计算题；首页题量、科目统计、搜索和复习范围均按公开题集计算。每题同时标注知识点、考查能力和难度，便于模拟结果归纳薄弱类型。
- 15 个零基础讲义科目用于系统理解概念、原理、特点、作用和基础例题。
- 15 个三色笔记科目按考频划分为红色高频、蓝色中频、黑色低频，每条包含知识点、常见考法和易错边界。
- 讲义与三色笔记分别承担“学懂”和“抓重点”的任务，便于从“看懂”过渡到“能答”。
...
3. 打开 `color-notes.html`，按红色高频、蓝色中频、黑色低频依次复习，重点核对常见考法和易错边界。
...
| 三色笔记 | 按考频抓重点、补边界 | 能说出高频结论、常见考法和易错边界 |
| 刷题 | 用客观题、简答自评和基础应用题检验掌握 | 能说明结论、原因或机制及适用边界 |

三个模块不是三份重复资料：讲义负责“学懂”，三色笔记负责“抓重点”，刷题负责“会答并纠错”。
...
- `color_notes/*.md`：15 个科目三色笔记内容的唯一可编辑数据源。
...
`dist/` 采用发布白名单，只包含 HTML、CSS、浏览器脚本、站点图标和题目配图。浏览器运行资源、讲义页面、三色笔记页面、首页索引和发布版本号均在构建时生成，不提交到版本库。
