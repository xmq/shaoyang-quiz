...
**通俗理解：** 样式像给“一级标题”“正文”等角色规定统一制服：某段被指定为一级标题，就同时获得对应的字体、间距和大纲级别；以后修改这套制服，所有使用该样式的段落会一起更新。这个类比对应的是“命名的格式集合加结构含义”，而手工加粗、放大只改变当前文字的外观。样式不能替作者判断一段文字究竟是不是标题，局部直接格式还可能覆盖样式中的默认外观。
...
**通俗理解：** 分页符像在笔记本中直接翻到下一张纸，只改变后续内容从哪里开始；分节符像在同一本文档中加一道可独立装修的隔断，使隔断两侧可以使用不同的纸张方向、页眉页脚或页码规则。类比的边界是“节”不一定另起一页：连续分节符可以留在同一页，只有下一页、奇数页等类型的分节符同时带来换页效果。
...
**通俗理解：** 复制公式像把一张“从当前位置向左取数量、再向左取单价”的操作说明交给下一行：相对引用记录的是位置关系，所以会随公式移动；绝对引用像钉在墙上的统一税率表，公式移到哪里都仍指向同一格。混合引用则只钉住行或列。`$` 只控制复制公式时地址怎样变化，并不会锁住单元格内容，也不能代替工作表保护。

**小例子：** 若 `B2` 是数量、`C2` 是单价、`F1` 是统一税率，`D2` 可写为 `=B2*C2*(1+$F$1)`。向下填充后，数量和单价会变为 `B3`、`C3`，税率仍引用 `$F$1`。
